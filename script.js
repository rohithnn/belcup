...
playerCategory.addEventListener('change', () => {
  const show = playerCategory.value === 'doubles';
  partnerGroup.style.display = show ? 'flex' : 'none';
  partnerGroup.style.flexDirection = show ? 'column' : '';
  partnerGroup.style.gap = show ? '6px' : '';
});
...
regForm.addEventListener('submit', (e) => {
  ...
  if (!name) return showFieldError('playerName', 'Please enter your full name.');
  if (!phone || !/^\d{10}$/.test(phone)) return showFieldError('playerPhone', 'Please enter a valid 10-digit number.');
  if (!age || age < 15) return showFieldError('playerAge', 'You must be at least 15 years old.');
  if (!category) return showFieldError('playerCategory', 'Please select a category.');
...
function showFieldError(id, msg) {
  ...
  tip.textContent = msg;
  tip.style.cssText = 'font-size:0.75rem;color:#ff6b6b;margin-top:2px;';
